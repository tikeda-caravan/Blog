...
事象Aと事象Bが互いに排反であるときには次の式が成り立ちます。[参考サイト](https://kou.benesse.co.jp/nigate/math/a14m0513.html){target="_blank"}
...
また、次の式が成り立つことを、２つの事象 $A$ , $B$ は独立であると言います。
...
---
Q. 離散型の確率変数の分散を求める式を2つ答えろ(うち一つは $p_i$ を用いて答えろ )
...
---
Q. 確率変数 $X$ の期待値と分散について $\{E(X)\}^2$, $E(X^2)$ を用いて公式を答えろ
...
---
Q. 二項分布の確率密度関数・期待値・分散を答えろ(確率は $p$ , サンプルサイズは $n$ とする)
...
- ポアソン分布は、 $\lambda$ が大きくなるにつれて正規分布に近づく( $n$ が10くらいでも正規分布)

---
Q. 幾何分布の確率密度関数・期待値・分散を答えろ (確率を $p$ とする)
...
---
標本平均を $\bar{X}$ , 母分散 $\sigma$ , 不偏分散を $U^2$ , サンプルサイズ $n$ を用いて、以下の問いに答えろ
...
Q. 母分散の推定(母平均未知)を求める公式を答えろ<sup>*</sup>
...
- 「信頼区間の幅が $\pm$ 95% 以下となるために必要なサンプルサイズ」の場合、  
  もしくは、「標本誤差をx%以内にするために必要なサンプルサイズ」を求める場合

$$1.96 \times \sqrt{\frac{\hat{p}(1-\hat{p})}{n}} \leq \frac{x}{100}$$
...
$$\frac{X_{diff}}{\sqrt{\frac{\sigma^2_1}{n_1} + \frac{\sigma^2_2}{n_2}}}$$
...
$$\frac{X_{diff}}{\sqrt{\frac{U^2_1}{n_1}+\frac{U^2_2}{n_2}}}$$
...
.pull-left[
$$\frac{X_{diff}}{\sqrt{U_p^2(\frac{U^2_1}{n_1}+\frac{U^2_2}{n_2})}}$$
]
...
---
Q. 適合度検定とは、どのようなときに用いる検定か答えろ

--

A. データの分布が特定の理論にあっていないことを示すための検定

Q. この時の、帰無仮説と対立仮説を答えろ
...
Q. 観測度数を $X_1, X_2, \cdots, X_k$ , 全度数を $n$ とするときの期待度数を求めろ
...
A. 期待度数 $p_{ij}$ は周辺度数の積を全度数で割って求められます。

$$p_i=\frac{X_{i.} \times X_{.j}}{n}$$
...
tbl2_que2 <- readr::read_csv(file = here::here("data/tbls2_qes2.csv"),
                             col_types = cols(A = col_character(),
                                              平方和 = col_character(),
                                              自由度 = col_character(),
                                              平均平方 = col_character(),
...
kableExtra::kbl(tbl2_que, col.names = c("", "偏差平方和", "自由度", "分散", "分散比"), escape = FALSE, format = "html", align = "ccccc") %>% 
  kableExtra::kable_styling() 

kableExtra::kbl(tbl2_que2, col.names = c("", "平方和", "自由度", "平均平方", "F値"), escape = FALSE, format = "html", align = "ccccc") %>% 
  kableExtra::kable_styling() 
```
...
tbl2_ans2 <- readr::read_csv(file = here::here("data/tbls2_ans2.csv"),
                             col_types = cols(A = col_character(),
                                              平方和 = col_character(),
                                              自由度 = col_character(),
                                              平均平方 = col_character(),
...
kableExtra::kbl(tbl2_ans, col.names = c("", "偏差平方和", "自由度", "分散", "分散比"), escape = FALSE, format = "html", align = "ccccc") %>% 
  kableExtra::kable_styling() 

kableExtra::kbl(tbl2_ans2, col.names = c("", "平方和", "自由度", "平均平方", "F値"), escape = FALSE, format = "html") %>% 
  kableExtra::kable_styling() 
```
...
Q. この時のt値を、回帰係数 $\beta$ と標準誤差 $se$ を使って求めろ
