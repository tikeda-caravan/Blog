...
Q. 母比率 $p$ の推定（大標本）を求める公式を答えろ ( $X/n$ )
...
- 「信頼区間の幅が $\pm$ 95% 以下となるために必要なサンプルサイズ」の場合、  
  もしくは、「標本誤差を $x$ %以内にするために必要なサンプルサイズ」を求める場合

$$1.96 \times \sqrt{\frac{\hat{p}(1-\hat{p})}{n}} \leq \frac{x}{100}$$

- （「2×」の部分は不要）

.footnote[<a href="https://bellcurve.jp/statistics/course/9129.html" target="_blank" rel="noopener noreferrer">参考サイト</a>]
...
---
Q. 回帰係数 $\beta$ の信頼区間を 標準誤差 $se$ 求めよ
...
.pull-right[
- 説明変数と残差の積和は0である（この式は、説明変数と残差との間には相関がない（＝無相関である）ということを表します）
  $$ \sum x_ie_i = 0$$
]
